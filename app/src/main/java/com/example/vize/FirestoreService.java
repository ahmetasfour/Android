...
public class FirestoreService {
    ...
    public FirestoreService() {
        ...
        currentUser = FirebaseAuth.getInstance().getCurrentUser();  // Ensure user is authenticated
    }

    public void addDailyRun(String date, float distance, OnCompleteListener<Void> listener) {
        if (currentUser == null) {
            return;  // User not logged in, handle as needed
        }
        ...
        db.collection("users").document(currentUser.getUid())
                .collection("dailyRuns").document(date)  // Using date as document ID to avoid duplicate entries per day
                .set(run)
